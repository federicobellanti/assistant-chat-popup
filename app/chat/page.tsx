...
export default function ChatPage() {
  ...
  const title = params.get('title') || 'Model Assistant';
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="p-3 bg-white border-t flex gap-2">
        <textarea className="flex-1 border rounded-xl p-2 text-sm" rows={2} value={input} onChange={e => setInput(e.target.value)} onKeyDown={onKey} placeholder="Type your question…" />
        <button className="px-3 py-2 rounded-xl bg-black text-white text-sm disabled:opacity-50" disabled={busy} onClick={send}>Send</button>
      </div>
    </div>
  );
}
